...
void qsort(int* mas, int* mas_index, int size) { // функция сортировки
	int i = 0;
	int j = size - 1;
	int mid = mas[size / 2];
	do {
...
int main()
{
	...
	cout << "\nВведите расстояния от работы до домов всех сотрудников (N положительных целых чисел, не превышающих 1000): " << endl;
	for (int i = 0; i < N; i++) {
		cout << i + 1 << ") ";
		cin >> mas1[i];
		mas1_index[i] = i;
	...
	cout << "\nВведите тарифы за проезд одного километра каждого такси (N положительных целых чисел, не превышающих 1000): " << endl;
	for (int i = 0; i < N; i++) {
		cout << i + 1 << ") ";
		cin >> mas2[i];
	}
